...
int _mosquitto_send_subscribe(struct mosquitto *mosq, int *mid, const char *topic, uint8_t topic_qos)
{
		...
		__android_log_print(ANDROID_LOG_INFO, "JNITag", "subcribe at send_client_mosq.c\r\n");
	#endif
...
{
	#ifdef PRINT_ANDROID_INFO
		__android_log_print(ANDROID_LOG_INFO, "JNITag", "unsubcribe at send_client_mosq.c\r\n");
	#endif
	/* FIXME - only deals with a single topic */
	struct _mosquitto_packet *packet = NULL;
	uint32_t packetlen;
