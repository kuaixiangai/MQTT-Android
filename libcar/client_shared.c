...
int JNU_ThrowByName(JNIEnv *env, char *java_exc_class, char *msg)
{
	...
	if(exc)/* 有jni异常时,打印异常信息,清除异常,抛出一个新的异常 */
	{
	#ifdef PRINT_ANDROID_INFO
		__android_log_print(ANDROID_LOG_INFO, "JNITag", "%s-->EnterJniException...\r\n", msg);
	#endif

		jclass new_exc;
		(*env)->ExceptionDescribe(env); /* 打印一个异常和堆栈跟踪信息 */
		(*env)->ExceptionClear(env); /* 清除异常 */
		new_exc = (*env)->FindClass(env, java_exc_class);

		if (new_exc != NULL)
		{
		#ifdef PRINT_ANDROID_INFO
			__android_log_print(ANDROID_LOG_INFO, "JNITag", "%s-->JniException: thrown from C code\r\n", msg);
		#endif
			(*env)->ThrowNew(env, new_exc, msg);/* 抛出新的异常 */
			(*env)->DeleteLocalRef(env, new_exc);/* 释放本地jclass */
		}
		return 1;
	}
	return 0;
...
int client_topic_set(struct mosq_config *cfg, int pub_or_sub, const char *topic, const int qos)
{
	if(pub_or_sub == CLIENT_PUB){
		...
		cfg->topic_qos[cfg->topic_count-1] = qos;
		//cfg->topic = strdup(topic);
	}
